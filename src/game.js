(() => {
  ...
  const PLAYER_SPRITE_SRC = `Outfit01.png`;
  const PLAYER_PUTT_SPRITE_SRC = `Outfit01_Putt.png`;
  const PLAYER_IDLE_SPRITE_SRC = `Outfit01_Idle.png`;
...
const HOLES = [
  ...
  const assets = {
    ...
    playerReady: false,
    playerPutt: null,
    playerPuttReady: false,
    playerIdle: null,
    playerIdleReady: false
  };
  assets.ball = loadImage(BALL_SPRITE_SRC);
  assets.player = loadImage(PLAYER_SPRITE_SRC, "playerReady");
  assets.playerPutt = loadImage(PLAYER_PUTT_SPRITE_SRC, "playerPuttReady");
  assets.playerIdle = loadImage(PLAYER_IDLE_SPRITE_SRC, "playerIdleReady");
  ...
  function computeLaunchFromDrag() {
    ...
    // More backspin on higher lofted shots
  ...
  function drawPlayer() {
    ...
    const isPutt = isPuttLie(world.ball);
    let playerAsset;
    let playerAssetReady;
    let frameIndex;
    
    if (p.animating) {
      // Swing animation
      playerAsset = isPutt ? assets.playerPutt : assets.player;
      playerAssetReady = isPutt ? assets.playerPuttReady : assets.playerReady;
      frameIndex = p.frame;
    } else if (p.aiming) {
      ...
      // Idle animation
      playerAsset = assets.playerIdle;
      playerAssetReady = assets.playerIdleReady;
      const IDLE_FPS = 5; // Halved from 10 to 5 for a calmer animation
      const IDLE_FRAMES = 12;
      frameIndex = Math.floor(p.idleTimer * IDLE_FPS) % IDLE_FRAMES;
    }

    if (!playerAssetReady || !playerAsset) return;
  ...
  function drawMinimap(width, height) {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(x0 - 10, y0 - 10, mapW + 20, mapH + 20);
    ctx.clip();
